...
| `image`            | Robust read-only disk imaging: copy a device/image to an image file, tolerating bad sectors (sector-granular retry, holes recorded), writing sparse output, and checkpointing to a map file so an interrupted copy can `--resume`. |
